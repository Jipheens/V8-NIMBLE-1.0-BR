...
class ClientMaintenancePage {
  ...
  initEmploymentBehaviour() {
    ...
    const setAmount = (name, value) => {
      ...
      field.value = Number(value.toFixed ? value.toFixed(2) : value);
    };
  ...
  getAddresses() {
    ...
    return Array.from(this.addressContainer.querySelectorAll("[data-collection-item]")).map((card) => ({
      ...
      ZipCode: card.querySelector("[name='ZipCode']")?.value || "",
      ...
      SubCityZone: card.querySelector("[name='SubCityZone']")?.value || "",
      Wereda: card.querySelector("[name='Wereda']")?.value || "",
      Kebele: card.querySelector("[name='Kebele']")?.value || "",
      HouseNumber: card.querySelector("[name='HouseNumber']")?.value || "",
      Language: card.querySelector("[name='Language']")?.value || "",
      PhoneWork: card.querySelector("[name='PhoneWork']")?.value || "",
      PhoneHome: card.querySelector("[name='PhoneHome']")?.value || "",
      Mobile: card.querySelector("[name='Mobile']")?.value || "",
      Email: card.querySelector("[name='Email']")?.value || "",
      FaxNo: card.querySelector("[name='FaxNo']")?.value || "",
      TINNumber: card.querySelector("[name='TINNumber']")?.value || "",
